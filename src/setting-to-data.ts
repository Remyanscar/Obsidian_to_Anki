...
export async function settingToData(app: App, settings: PluginSettings, fields_dict: Record<string, string[]>): Promise<ParsedSettings> {
    ...
    result.multiline_yaml_tags = settings.Defaults["Multiline YAML Tags"] ?? false
    ...
    if (result.multiline_yaml_tags) {
        result.TAG_REGEXP = new RegExp(String.raw`^` + escapeRegex(settings.Syntax["File Tags Line"]) + String.raw`:\s*\n((?:^[ \t]*-[ \t]+.*\n?)+)`, "m")
    } else {
        result.TAG_REGEXP = new RegExp(String.raw`^` + escapeRegex(settings.Syntax["File Tags Line"]) + String.raw`(?:\n|: )(.*)`, "m")
    }

    result.NOTE_REGEXP = new RegExp(String.raw`^` + escapeRegex(settings.Syntax["Begin Note"]) + String.raw`([\s\S]*?\n)` + escapeRegex(settings.Syntax["End Note"]), "gm")
    ...
    result.ignored_metadata_regexps = settings.IGNORED_METADATA_REGEXPS ?? [];
